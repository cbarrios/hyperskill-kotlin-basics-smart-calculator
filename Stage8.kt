...
fun getOperandValue(operand: String): String? {
...
fun processSingleString(first: String) {
    when (first) {
        ...
        "/help" -> println("The program calculates addition, subtraction, multiplication, division and power of numbers (and big too) and/or variables with and without parentheses")
        else -> {
            if (first.first() == '/') {
...
fun calculateFromPostfix(postfix: String) {
    ...
    for (e in list) {
        if (!isOperator(e)) {
            ...
            when (getOperator(e)) {
                Operator.ADD -> stack.addFirst("${n2.toBigInteger() + n1.toBigInteger()}")
                Operator.SUBTRACT -> stack.addFirst("${n2.toBigInteger() - n1.toBigInteger()}")
                Operator.MULTIPLY -> stack.addFirst("${n2.toBigInteger() * n1.toBigInteger()}")
                Operator.DIVIDE -> stack.addFirst("${n2.toBigInteger() / n1.toBigInteger()}")
                Operator.POWER -> stack.addFirst("${n2.toBigInteger().pow(n1.toInt())}")
                Operator.NULL -> Unit
            }
        }
    }
